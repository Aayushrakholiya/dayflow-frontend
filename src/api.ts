...
*  DESCRIPTION   : 
*    This file defines the API functions for interacting with the backend services in the Dayflow application.
*/ 
...
// ===== EVENT INVITES =====
export async function sendEventInvites(payload: {
  eventTitle: string;
  date: string;
  startHour: number;
  endHour: number;
  ...
  const res = await fetch(`${API_URL}/api/events/send-invites`, {
    method: "POST",
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    console.error("Failed to send event invites:", body);
  }
}
